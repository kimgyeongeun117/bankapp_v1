...
public class AccountService {
	...
	@Transactional
	public void updateAccountDeposite(DepositeFormDto depositeFormDto) {
		
		Account accountEntity = accountRepository.findByNumber(depositeFormDto.getDAccountNumber());
		if(accountEntity == null) {
			throw new CustomRestfulException("해당 계좌가 존재하지 않습니다.", HttpStatus.INTERNAL_SERVER_ERROR);
		}
		
		// 객체 상태 값 변경
		accountEntity.deposite(depositeFormDto.getAmount());
		accountRepository.updateById(accountEntity);
		
		History history = new History();
		history.setAmount(depositeFormDto.getAmount());
	...
	public void updateAccountTransfer(TransferFormDto transferFormDto,Integer principalId) {
		...
		// 2
		Account depositeAccountEntity = accountRepository.findByNumber(transferFormDto.getDAccountNumber());
		if(depositeAccountEntity == null) {
		...
		depositeAccountEntity.deposite(transferFormDto.getAmount());
		// 변경된 입금 객체 상태값으로 DB update 처리
		accountRepository.updateById(depositeAccountEntity);
		// 8
		History history = new History();
		history.setAmount(transferFormDto.getAmount());
		history.setWAccountId(withdrawAccountEntity.getId());
		history.setDAccountId(depositeAccountEntity.getId());
		history.setWBalance(withdrawAccountEntity.getBalance());
		history.setDBalance(depositeAccountEntity.getBalance());
